...
public class PlayerWhite extends AbstractPlayer
{
	public PlayerWhite()
	{
	...
	{
		glPushMatrix();
		glBindTexture(GL_TEXTURE_2D, Textures.PLAYER_LIFE.getTextureID());
		glTranslated(xTrans, yTrans, 0);
		glBegin(GL_QUADS);
		{;
			glTexCoord2d(0, 0);
			glVertex2d(x - (Textures.PLAYER_LIFE.getImageWidth() / 2), y - (Textures.PLAYER_LIFE.getImageHeight() / 2));
	...
	public void translateY(double trans)
	{
		...
		if(y + yTrans + trans + (bounds.height / 2) >= Display.getHeight()) return;
